...
qj=pd.read_csv('data7\data_qj.csv',encoding='latin1')
xk=pd.read_csv('data7\data_xk.csv',encoding='latin1')
lyf=pd.read_csv('data7\data_lyf.csv',encoding='latin1')
czk=pd.read_csv('data7\data_czk.csv',encoding='latin1')
cl=pd.read_csv('data7\data_cl.csv',encoding='latin1')
yyh=pd.read_csv('data7\data_yyh.csv',encoding='latin1')
db=pd.read_csv('data7\data_db.csv',encoding='latin1')
gwc=pd.read_csv('data7\data_gwc.csv',encoding='latin1')
syp=pd.read_csv('data7\data_syp.csv',encoding='latin1')
txr=pd.read_csv('data7\data_txr.csv',encoding='latin1')
cjq=pd.read_csv('data7\data_cjq.csv',encoding='latin1')
yyt=pd.read_csv('data7\data_yyt.csv',encoding='latin1')
...
with tf.Session() as sess:
    ...
    for jj in range(10000):
        answer = sess.run(prediction, feed_dict={xs: TestingInput})
        answer2 = sess.run(prediction, feed_dict={xs: TrainingInput})
        ...
        trainaccuarcy.append(tt)
        testaccuarcy.append(totallyaccuracy)
        if jj%1000==0:
            print('TrainingTime:')
            print(jj)
    saver.save(sess, "./modelTT.ckpt")
with tf.Session() as sessss:
    saver.restore(sessss, "./modelTT.ckpt")
...
print(D2)
fig1, =plt.plot(trainaccuarcy,label='Training Accuarcy')
fig2, =plt.plot(testaccuarcy,label='Testing Accuracy')
plt.xlim(xmax=10000,xmin=0)
plt.ylim(ymax=1,ymin=0)
# plt.legend([fig1],['Training Accuracy'])
# plt.legend([fig2],['Testing Accuracy'])
